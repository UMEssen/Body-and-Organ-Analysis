...
roi_groups = {
    ...
    "lung_vessels": [["lung_trachea_bronchia"], ["lung_vessels"]],
    "covid": [["lung_covid_infiltrate"]],
    ...
    for idx, roi in enumerate(rois):
        ...
        if data.max() > 0:  # empty mask
            ...
            cont_actor = plot_mask(
                scene, data, affine, x, y, smoothing=smoothing, color=color, opacity=1
            )
...
def plot_subject(
    ct_img, output_path, df=None, roi_data=None, smoothing=20, task_name="total"
):
    subject_width = 330
    # subject_height = 700
    nr_cols = 10
    ...
    window.record(
        scene, size=window_size, out_path=output_path, reset_camera=False
    )  # , reset_camera=False
    scene.clear()
