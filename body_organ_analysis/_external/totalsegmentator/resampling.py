...
def resample_img_cucim(img, zoom=0.5, order=0, nr_cpus=-1):
    ...
    resampled_img = cp.asnumpy(
        resampled_img
    )  # Alternative: img_arr = cp.float32(resampled_img.get())   # very fast
...
def resample_img_nnunet(data, mask=None, original_spacing=1.0, target_spacing=2.0):
    ...
    data_res, mask_res = resample_patient(
        data, mask, original_spacing, target_spacing, force_separate_z=None
    )
