...
def search_optimal_ray_angle(
    ...
    for phi in angles:
        ...
        mask = np.logical_and(mask, bone_mask)
...
def find_intersection_start_end_points_on_ray(
    ...
    linemask = np.zeros(mask.shape, dtype=bool)
    rr, cc = skimage.draw.line(
        *sample_pos1.round().astype(int), *sample_pos2.round().astype(int)
    )
    linemask[rr, cc] = True

    intersection_mask = np.logical_and(linemask, mask)
