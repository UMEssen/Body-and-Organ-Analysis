...
* In totalsegmentator/nnunet.py:
  ...
  * In lines 270-273 some properties of the niftis were changed to avoid the "ITK ERROR: ITK only supports orthonormal direction cosines.  No orthonormal definition found!" error.
  ...
  * The force_split check in line 282 was changed to ensure that the image has at least 200px in the z-axis.
  * The resample to original resolution and to the original axes has been changed accordingly to the previous modifications.
  * The postprocessing of the lung_vessels task has been changed in lines 534-547 such that if the original crop input was a numpy array, it can be be used for the postprocessing.
